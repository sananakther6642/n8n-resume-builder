...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/compile') {
    ...
    req.on('end', () => {
      try {
        ...
        console.log(`[${new Date().toISOString()}] Compiled OK — PDF size: ${result.pdfBase64.length} base64 chars`);

        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify(result));
      } catch (err) {
        console.error(`[${new Date().toISOString()}] Compile error:`, err.message);
  ...
  res.end(JSON.stringify({ error: 'Not found. Use POST /compile or GET /health' }));
});
